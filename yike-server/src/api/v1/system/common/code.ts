...
const devEnvAdditionalOptions = {
  mathMin: 6,
  mathMax: 6,
}

const generateOptionsByENV = (env: string) => {
  return env === 'development'
    ? {
        ...captchaDefaultOptions,
        ...devEnvAdditionalOptions,
      }
    : captchaDefaultOptions
}
...
router.get('/code', async (ctx: Models.Ctx) => {
  const captcha: svgCaptcha.CaptchaObj = svgCaptcha.createMathExpr(
    generateOptionsByENV(process.env.NODE_ENV!),
  )
