...
router.get('/query', verifyToken, async (ctx: Models.Ctx) => {
  ...
  AllMenulist.forEach((element: MenuList) => {
    ...
    const data = filterMenuList.find(
      (item) =>
        info.nickName === item.info.nickName &&
  ...
  // 获取所有的操作(即按钮)
  const allActions: Permissions[] = menuList.filter(
    (item) => item.menuType === 3,
  )
  // 获取所有的菜单目录和菜单
  const allMenu: Permissions[] =
    menuList.filter((item) => item.menuType === 1 || item.menuType === 2) || []
  // 根据parentId给菜单添加操作
